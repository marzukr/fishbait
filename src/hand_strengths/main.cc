...
int main() {
  ...
  // hand_strengths::SaveLUT("luts/showdown_lut_64.cereal", &showdown_lut,
  //     true);

  // Load Showdown LUT
  // std::vector<hand_strengths::ShowdownStrength> showdown_lut;
  // hand_strengths::LoadLUT("luts/showdown_lut_64.cereal", &showdown_lut,
  //     true);

  // Generate Preflop LUT
  // utils::Matrix<uint32_t> preflop_lut = PreflopLUT(showdown_lut, true);
  // hand_strengths::SaveLUT("luts/preflop_lut_64.cereal", &preflop_lut, true);

  // Load Preflop LUT
  // utils::Matrix<uint32_t> preflop_lut(1, 1, 0);
  // hand_strengths::LoadLUT("luts/preflop_lut_64.cereal", &preflop_lut, true);
  ...
  // hand_strengths::SaveLUT("luts/flop_lut_64.cereal", &flop_lut, true);

  // Load Flop LUT
  // utils::Matrix<uint32_t> flop_lut(1, 1, 0);
  // hand_strengths::LoadLUT("luts/flop_lut_64.cereal", &flop_lut, true);
  ...
  // hand_strengths::SaveLUT("luts/turn_lut_64.cereal", &turn_lut, true);

  // Load Turn LUT
  // utils::Matrix<uint32_t> turn_lut(1, 1, 0);
  // hand_strengths::LoadLUT("luts/turn_lut_64.cereal", &turn_lut, true);
  ...
  // hand_strengths::SaveLUT("luts/river_lut_64.cereal", &river_lut, true);

  // Load River LUT
  // utils::Matrix<double> river_lut(1, 1, 0);
  // hand_strengths::LoadLUT("luts/river_lut_64.cereal", &river_lut, true);
